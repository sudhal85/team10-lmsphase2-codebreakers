...
public class BatchStepDefinition 
{
	...
	//static Logger log =  LogManager.getLogger(BatchStepDefinition.class);
	...
	{
		LoggerLoad.info("=========== Validate landing in Batch page =====================  ");
	...
	{
		LoggerLoad.info("=========== Validate header in the Batch Page =====================  ");
	...
	{
		LoggerLoad.info("=========== Validate pagination in the Batch Page =====================  ");
	...
	{
		LoggerLoad.info("===========  Validate data table headers in the Batch Page =====================  ");
	...
	{
		LoggerLoad.info("===========  Validate Delete button in Batch Page =====================  ");
	...
	{
		LoggerLoad.info("===========  Validate A New batch in Batch Page =====================  ");
	...
	{
		LoggerLoad.info("===========  Validate data rows =====================  ");
	...
	{
		LoggerLoad.info("===========  Validate pop up for adding batch =====================  ");
	...
	{
		LoggerLoad.info("===========  Check if the fields exist in pop up =====================  ");
	...
	{
		LoggerLoad.info("===========  Check if description is optional field =====================  ");
		Thread.sleep(1000);
	...
	{
		LoggerLoad.info("===========  Check if the program details are added in data table =====================  ");
	...
	{
		LoggerLoad.info("===========  Check for error messages for invalid fields =====================  ");
	...
	{
		LoggerLoad.info("===========  Check for error messages for mandatory fields =====================  ");
	...
	{
		LoggerLoad.info("===========  Validate row level edit icon =====================  ");
		ExcelReader excelReader = new ExcelReader();
		List<Map<String,String>> testData = excelReader.getData("./src/test/resources/TestData/Data1.xlsx", SheetName);
		String name = testData.get(RowNumber).get("Name");
		batchobj.getBatchSearchBox().sendKeys(name);
	...
	{
		LoggerLoad.info("===========  Check if the fields are updated =====================  ");
		Assert.assertEquals(batchobj.getSuccessMsg().getText(), msg);
		ExcelReader excelReader = new ExcelReader();
		List<Map<String,String>> testData = excelReader.getData("./src/test/resources/TestData/Data1.xlsx", SheetName);
		String name = testData.get(RowNumber).get("Name");
	...
	{
	   LoggerLoad.info("===========  Check if the update throws error with invalid values =====================  ");
	...
	{
	   LoggerLoad.info("===========  Check if you get error message when mandatory fields are erased =====================  ");
	...
	{
		LoggerLoad.info("===========  Check if description field is optional in update =====================  ");
		ExcelReader excelReader = new ExcelReader();
		List<Map<String,String>> testData = excelReader.getData("./src/test/resources/TestData/Data1.xlsx", SheetName);
		String name = testData.get(RowNumber).get("Name");
	...
	{
		LoggerLoad.info("===========   Validate row level delete icon =====================  ");
	...
	{
		LoggerLoad.info("===========  Validate accept alert =====================  ");
		Assert.assertEquals(batchobj.getSuccessMsg().getText(), msg);
	...
	{
		LoggerLoad.info("=========== Validate reject alert =====================  ");
		ExcelReader excelReader = new ExcelReader();
		List<Map<String,String>> testData = excelReader.getData("./src/test/resources/TestData/Data1.xlsx", SheetName);
		String name = testData.get(RowNumber).get("Name");
	...
	{
		batchobj.clickOnBatch();
		Assert.assertTrue(batchobj.verifyCheckBoxeDisabled().TRUE);
	}
	...
	{
		LoggerLoad.info("===========  Validate the delete icon below the header =====================  ");
		Assert.assertTrue(batchobj.batchHeaderDeleteIcon().isDisplayed());
		Assert.assertEquals(header, batchobj.getBatchHeader());
	}
	
	@Given("admin enters data from {string} and RowNumber {int} One of the checkbox or row is selected")
	public void admin_enters_data_from_and_row_number_one_of_the_checkbox_row_is_selected(String SheetName, int RowNumber) throws InvalidFormatException, IOException, InterruptedException
	{
		batchobj.clickOnBatch();
		ExcelReader excelReader = new ExcelReader();
		List<Map<String,String>> testData = excelReader.getData("./src/test/resources/TestData/Data1.xlsx", SheetName);
		...
		batchobj.getBatchSearchBox().sendKeys(name);
		Thread.sleep(1000);
		batchobj.batchClickOnCheckbox();
	}

	@When("Click delete icon below {string} header")
	public void click_delete_icon_below_header(String header)
	{
		Assert.assertEquals(header, batchobj.getBatchHeader());
		batchobj.batchHeaderDeleteIcon().click();
		batchobj.clickOnYesInDelete();
	}
	...
	{
		LoggerLoad.info("===========  Check for single row delete =====================  ");
		Thread.sleep(1000);
		Assert.assertEquals(batchobj.batchPaginationDetails().getText(), Constants.PAGINATION_DETAILS);
	}
	
	@Given("admin enters data from {string} and RowNumber {int} Two or more checkboxes or row is selected")
	public void admin_enters_data_from_and_row_number_two_or_more_checkboxes_or_row_is_selected(String SheetName, int RowNumber) throws InvalidFormatException, IOException, InterruptedException 
	{
		batchobj.clickOnBatch();
		ExcelReader excelReader = new ExcelReader();
		List<Map<String,String>> testData = excelReader.getData("./src/test/resources/TestData/Data1.xlsx", SheetName);
		...
		batchobj.getBatchSearchBox().sendKeys(name);
		Thread.sleep(1000);
		batchobj.batchClickCommonCheckbox();
	}

	@When("Click the delete icon below {string} header")
	public void click_the_delete_icon_below_header(String header) 
	{
		Assert.assertEquals(header, batchobj.getBatchHeader());
		batchobj.batchHeaderDeleteIcon().click();
		batchobj.clickOnYesInDelete();
	...
	public void the_respective_row_from_the_data_table_is_deleted() throws InterruptedException 
	{
		LoggerLoad.info("===========  Check for multi row delete =====================  ");
		Thread.sleep(1000);
		Assert.assertEquals(batchobj.batchPaginationDetails().getText(), Constants.PAGINATION_DETAILS);
	}	
